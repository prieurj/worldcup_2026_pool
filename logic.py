...
def update_username(user_id: str, new_username: str) -> bool:
    ...
    if existing.data:
        return False
    sb.table("users").update({"username": new_username}).eq("username", user_id).execute()
    return True


def get_payment_status() -> dict:
    sb = get_supabase()
    resp = sb.table("payment_status").select("username, paid").execute()
    return {r["username"]: r["paid"] for r in resp.data}


def set_payment_status(username: str, paid: bool):
    sb = get_supabase()
    sb.table("payment_status").upsert({"username": username, "paid": paid}).execute()
